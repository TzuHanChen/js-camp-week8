...
async function getProductsByCategory(category) {
  ...
  return products.reduce((prev, cur) => {
    return (cur.category === category) ? [...prev, cur] : prev;
  }, []);
}
...
function displayProducts(products) {
  ...
  products.forEach((product, index) => {
    ...
    console.log(`${spaces}分類：${product.category}`);
    console.log(`${spaces}原價：${formatCurrency(product.price)}`);
    console.log(`${spaces}售價：${formatCurrency(product.origin_price)} (${getDiscountRate(product)})`);
    console.log('----------------------------------------');
  });
}
